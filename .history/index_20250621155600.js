...
class Book{
    ...
    getDetails(){
        console.log(`${this.title} by ${this.author}, (ISBN: ${this.id}), ${this.availableQuantity}/${this.quantity} available`)
    }
}
...
class Library{
    ...
    addBook(book){
        ...
        if(!inBooks){
            ...
            console.log(`${book.title} by ${book.author} is added succesfully`)
        }
        else{
            console.log(`${book.title} by ${book.author} is already added`)
        }
    }
    ...
    regiterMember(member){
        ...
        if(!inMember){
            ...
            console.log(`Name: ${member.name},ID: ${member.id} is regitered succesfully`)
        }
        else{
            console.log(`Name: ${member.name}, ID: ${member.id} is already regitered`)
        }
    }

    regiterLibrarian(libraian){
        const inLibrarian=this.libraian.find(l=>l.id===libraian.id)
        if(!inLibrarian){
            this.libraian.push(libraian)
            console.log(`Name: ${libraian.name}, ID: ${libraian.id} is added as a librarian`)
        }
        else{
            console.log(`Name: ${libraian.name}, ID: ${libraian.id} is already librarian`)
        }
    }

    findBook(bookId){
      return this.books.find(book=>book.id===boo)
    }
}